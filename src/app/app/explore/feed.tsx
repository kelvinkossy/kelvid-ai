...
function ExploreCard({ job, user, isLiked, onLike }: { job: any; user: any; isLiked: boolean; onLike: () => void }) {
        ...
        <div className="flex items-center justify-between mt-2.5 pt-2.5 border-t border-border">
          ...
          <div className="flex items-center gap-2.5">
            <button onClick={onLike} className="flex items-center gap-1 text-[10px] text-text-tertiary hover:text-white transition-colors">
              ...
              <span>{job.likes + (isLiked ? 1 : 0)}</span>
            </button>
...
export default function ExploreFeed({ jobs, users }: { jobs: any[]; users: any[] }) {
  ...
  const toggleLike = (id: string) => setLikedSet(p => { const n = new Set(p); n.has(id) ? n.delete(id) : n.add(id); return n; });
  ...
  return (
    <div className={`grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 ${mounted ? "animate-fade-in" : ""}`}>
      {jobs.map((job, i) => {
        ...
        return <ExploreCard key={job.id} job={job} user={user} isLiked={likedSet.has(job.id)} onLike={() => toggleLike(job.id)} />;
      })}
    </div>
  );
}
